...
const ADHOC_AGENT_ID = "69787126a75ef8a94cc4f0d1"
...
export default function Home() {
  ...
  const sendChatMessage = async () => {
    ...
    try {
      const result = await callAIAgent(chatInput, ADHOC_AGENT_ID)

      if (result.success && result.response.status === 'success') {
        const data = result.response.result as AdHocResponse
  ...
  const deleteSelectedEmails = async () => {
    ...
    try {
      ...
      const result = await callAIAgent(message, ADHOC_AGENT_ID)
